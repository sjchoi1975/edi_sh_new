-- 흡수율 계산 함수 수정 - 100배 오류 해결
-- 기존 데이터 정규화 + 함수 수정

-- =====================================================
-- 1단계: 기존 데이터 정규화 (100으로 나누기)
-- =====================================================

UPDATE performance_records_absorption 
SET absorption_rate = absorption_rate / 100 
WHERE absorption_rate > 1;  -- 1보다 큰 값들만 정규화 (이미 소수점인 값은 제외)

-- =====================================================
-- 2단계: 수정된 흡수율 계산 함수 생성
-- =====================================================

-- 기존 함수 삭제
DROP FUNCTION IF EXISTS public.calculate_absorption_rates(text);

-- 새로운 함수 생성 (100을 곱하지 않음)
        ...
        SET 
            ...
            absorption_rate = CASE 
                WHEN result_record.prescription_amount > 0 THEN 
                    ROUND((COALESCE(result_record.wholesale_revenue, 0) + COALESCE(result_record.direct_revenue, 0)) / result_record.prescription_amount, 4)
                ELSE 0 
            END,
            analysis_time = NOW()
...
-- 수정 전후 데이터 확인
SELECT 
    '수정 전 후 데이터 상태' as info,
    COUNT(*) as total_records,
    COUNT(CASE WHEN absorption_rate > 1 THEN 1 END) as over_100_percent,
    COUNT(CASE WHEN absorption_rate <= 1 THEN 1 END) as normal_percent,
    AVG(absorption_rate) as avg_absorption_rate
FROM performance_records_absorption 
WHERE absorption_rate > 0;
