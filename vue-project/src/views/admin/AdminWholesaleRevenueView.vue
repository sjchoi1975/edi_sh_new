...
const downloadTemplate = async () => {
  ...
  const worksheet = workbook.addWorksheet('도매매출템플릿')
  ...
  link.href = url
  link.download = '도매매출자료_엑셀등록_템플릿.xlsx'
  link.click()
...
const downloadExcel = async () => {
  ...
  try {
    ...
    // ExcelJS 워크북 생성
    const workbook = new ExcelJS.Workbook()
    const worksheet = workbook.addWorksheet('도매매출목록')
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = generateExcelFileName('도매매출목록')
    link.click()
    window.URL.revokeObjectURL(url)
  } catch (error) {
