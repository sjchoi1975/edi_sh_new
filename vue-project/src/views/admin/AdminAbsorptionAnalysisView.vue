<template>
  <div class="absorption-analysis-view page-container" style="display: flex; flex-direction: column; height: 100vh;">
        ...
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <div class="company-search-container" style="position: relative;">
            <input
              ...
              :placeholder="selectedCompanyId === 'ALL' ? '업체명을 입력하세요...' : ''"
              class="select_200px"
              autocomplete="off"
            />
            <div v-if="showCompanyDropdown && filteredCompanies.length > 0" class="company-dropdown">
              ...
              <div
                :class="['company-dropdown-item', { 
                  selected: selectedCompanyId === 'ALL', 
                  highlighted: companyHighlightedIndex === 0 
                }]"
                @mousedown.prevent="selectCompany({ id: 'ALL', company_name: '전체' })"
              >
                전체
              </div>
              <!-- 업체 목록 -->
        ...
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <div class="hospital-search-container" style="position: relative;">
            <input
              ...
              :placeholder="selectedHospitalId === 'ALL' ? '병의원명을 입력하세요...' : ''"
              class="select_240px"
              autocomplete="off"
            />
            <div v-if="showHospitalDropdown && filteredHospitals.length > 0" class="hospital-dropdown">
              ...
              <div
                :class="['hospital-dropdown-item', { 
                  selected: selectedHospitalId === 'ALL', 
                  highlighted: hospitalHighlightedIndex === 0 
                }]"
                @mousedown.prevent="selectHospital({ id: 'ALL', name: '전체' })"
              >
                전체
              </div>
              <!-- 병의원 목록 -->
    ...
    <div class="data-card" style="flex-grow: 1; display: flex; flex-direction: column; overflow: hidden;">
      ...
      <div style="flex-grow: 1; overflow: hidden;">
        <DataTable 
          ...
          :pt="{
            wrapper: { style: 'min-width: 3100px;' },
        ...
        >
          ...
          <Column field="applied_absorption_rate" header="반영 흡수율" :headerStyle="{ width: columnWidths.applied_absorption_rate }" :sortable="true">
            <template #body="slotProps">
              <div class="absorption-rate-cell">
                <span :title="slotProps.data.review_action === '삭제' ? '0%' : slotProps.data.applied_absorption_rate">
                  {{ slotProps.data.review_action === '삭제' ? '0%' : slotProps.data.applied_absorption_rate }}
                </span>
                <button 
                  v-if="slotProps.data.review_action !== '삭제'"
                  @click="editAbsorptionRate(slotProps.data)"
                  class="edit-absorption-rate-btn"
                  title="반영 흡수율 수정"
                >
                  ✎
                </button>
    ...
    <!-- 반영 흡수율 수정 모달 -->
    <div v-if="showAbsorptionRateModal" class="modal-overlay" @click="cancelAbsorptionRateEdit">
      <div class="modal-content modal-center" @click.stop>
        <div class="modal-header">
          <h2>반영 흡수율 수정</h2>
          <button class="modal-close-btn" @click="cancelAbsorptionRateEdit">×</button>
        </div>
        <div class="modal-body">
          <div style="margin-bottom: 20px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 600;">지급액</label>
            <div style="padding: 10px; background-color: #f8f9fa; border: 1px solid #ddd; border-radius: 4px; font-size: 16px; font-weight: 600;">
              {{ editingRowData ? Math.round(Number(String(editingRowData.payment_amount).replace(/,/g, '')) || 0).toLocaleString() : '0' }}원
            </div>
          </div>
          <div>
            ...
            <div style="margin-top: 8px; font-size: 13px; color: #666;">
              지급액 × 반영 흡수율(%)로 최종 지급액이 계산됩니다.
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn-save" @click="saveAbsorptionRate">저장</button>
          <button class="btn-close" @click="cancelAbsorptionRateEdit">닫기</button>
        </div>
      </div>
    </div>
  </div>
...
const columnWidths = {
  ...
  commission_rate: '5%',
  payment_amount: '6%',
  applied_absorption_rate: '6%',
  final_payment_amount: '6%',
  remarks: '8%',
  created_date: '7%',
  created_by: '8%',
  updated_date: '7%',
...
async function fetchClientsForMonth(companyId = null) {
    ...
    try {
        ...
        // 이름순으로 정렬
        const sortedClients = uniqueHospitals.sort((a, b) => a.name.localeCompare(b.name));
        hospitalOptions.value = [{ id: 'ALL', name: '- 전체 -' }, ...sortedClients];
        selectedHospitalId.value = 'ALL';
    } catch (err) {
        console.error('병의원 조회 오류:', err);
        hospitalOptions.value = [{ id: 'ALL', name: '- 전체 -' }];
    }
}
...
function selectCompany(company) {
    ...
    companySearchText.value = company.id === 'ALL' ? '' : company.company_name;
  ...
  switch (event.key) {
    ...
    case 'Enter':
      ...
      if (companyHighlightedIndex.value === 0) {
        // "전체" 선택
        selectCompany({ id: 'ALL', company_name: '전체' });
    ...
    const mappedData = allData.map((row, i) => {
        try {
            ...
            return {
                ...
                applied_absorption_rate: `${appliedAbsorptionRate.toFixed(1)}%`,
                final_payment_amount: finalPaymentAmount.toLocaleString(),
            ...
            return {
                ...
                updated_date: null,
                updated_by: null,
                isPromotionRateApplied: false, // 오류 발생 시 기본값
            };
        }
    });
...
watch(selectedCompanyId, async (newCompanyId) => {
  ...
  if (newCompanyId === 'ALL') {
    ...
    const selectedCompany = allCompanies.value.find(c => c.id === newCompanyId);
    if (selectedCompany) {
...
const calculateAbsorptionRates = async () => {
  ...
  try {
    ...
    if (productsError) {
      console.warn('제품 정보 조회 오류 (프로모션 수수료 확인 생략):', productsError);
    }
    ...
    for (let i = 0; i < allSourceData.length; i += insertBatchSize) {
      ...
      const { error: insertError } = await supabase
        ...
        .insert(batchData.map(record => {
          ...
          // 프로모션 수수료 확인
          let commissionRate = record.commission_rate ? 
          ...
          // 프로모션 수수료 적용 확인
          const insuranceCode = productInsuranceCodeMap.get(record.product_id);
          if (insuranceCode) {
...
async function downloadExcel() {
  ...
  try {
    ...
    const dataToExport = displayRows.value.map((row, index) => ({
      ...
      '반영 흡수율': (row.applied_absorption_rate ? parseFloat(String(row.applied_absorption_rate).replace('%', '')) / 100 : 0),
      '최종 지급액': Math.round(Number(String(row.final_payment_amount).replace(/,/g, '')) || 0),
      '비고': row.remarks,
    ...
    const totalFinalPaymentAmountForExcel = displayRows.value.reduce((sum, row) => {
      // 삭제된 건은 최종 지급액을 0으로 계산
      if (row.review_action === '삭제') return sum;
      return sum + (Number(String(row.final_payment_amount).replace(/,/g, '')) || 0);
    }, 0);
    ...
    dataToExport.slice(0, -1).forEach((row) => {
      ...
      dataRow.eachCell((cell, colNumber) => {
        ...
        // 반영 흡수율 (19번째 컬럼): 백분율, 소수점 한자리
        if (colNumber === 19) {
          cell.numFmt = '0.0%'
        }
        
        // 최종 지급액 (20번째 컬럼): 천단위 콤마, 소수점 없음
        if (colNumber === 20) {
          cell.numFmt = '#,##0'
        }
      })
    ...
    totalRow.getCell(19).numFmt = '0.0%'; // 반영 흡수율
    totalRow.getCell(20).numFmt = '#,##0'; // 최종 지급액
...
async function saveAbsorptionRate() {
  ...
  try {
    ...
    // 해당 행의 데이터 업데이트
    const rowIndex = displayRows.value.findIndex(row => row.id === editingRowData.value.id);
    if (rowIndex !== -1) {
      // 반영 흡수율 업데이트
      ...
      if (displayRows.value[rowIndex].commission_rate) {
        const commissionString = String(displayRows.value[rowIndex].commission_rate).replace('%', '').trim();
        if (commissionString) {
          const parsedCommission = parseFloat(commissionString);
          if (!isNaN(parsedCommission)) {
            commissionRate = parsedCommission / 100;
          }
        }
      }
...
.edit-absorption-rate-btn:hover {
  background-color: #f8f9fa;
  color: #0056b3;
}


/* 정산 분배 페이지와 동일한 모달 스타일 */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
