<template>
  <div class="performance-register-view page-container">
    ...
    <div class="data-card">
      ...
      >
        <template #empty>{{ selectedSettlementMonth ? '등록된 실적이 없습니다.' : '정산월을 선택하세요.' }}</template>
        ...
        <!-- 업체 정보 -->
        <Column field="company_name" header="업체명" :headerStyle="{ width: columnWidths.company_name, textAlign: 'center' }">
          <template #body="slotProps">
            <span style="font-weight: 400;">{{ slotProps.data.company_name }}</span>
          </template>
        </Column>
        
        <Column field="business_registration_number" header="사업자등록번호" :headerStyle="{ width: columnWidths.business_registration_number, textAlign: 'center' }" />
        
        <Column field="representative_name" header="대표자" :headerStyle="{ width: columnWidths.representative_name, textAlign: 'center' }" />

        <!-- 실적 정보 -->
        <Column header="총 거래처" :headerStyle="{ width: columnWidths.total_clients, textAlign: 'center' }">
          <template #body="slotProps">
            {{ slotProps.data.total_clients ? slotProps.data.total_clients : 0 }}
          </template>
        </Column>
        
        <Column header="제출 거래처" :headerStyle="{ width: columnWidths.submitted_clients, textAlign: 'center' }">
          <template #body="slotProps">
            {{ slotProps.data.submitted_clients ? slotProps.data.submitted_clients : 0 }}
          </template>
        </Column>
        
        <Column header="처방건수" :headerStyle="{ width: columnWidths.prescription_count, textAlign: 'center' }">
          <template #body="slotProps">
            {{ slotProps.data.prescription_count ? slotProps.data.prescription_count : 0 }}
          </template>
        </Column>
        
        <Column field="prescription_amount" header="처방액" :headerStyle="{ width: columnWidths.prescription_amount, textAlign: 'center' }" :bodyStyle="{ textAlign: 'right' }" :sortable="true">
          <template #body="slotProps">
            {{ formatNumber(slotProps.data.prescription_amount) }}
          </template>
        </Column>
        <Column field="evidence_files" header="증빙 파일" :headerStyle="{ width: columnWidths.evidence_files, textAlign: 'center' }" :bodyStyle="{ textAlign: 'center' }" :sortable="true">
          <template #body="slotProps">
            {{ slotProps.data.evidence_files || 0 }}
          </template>
        </Column>
        <Column header="파일 보기" :headerStyle="{ width: columnWidths.file_view, textAlign: 'center' }" :bodyStyle="{ textAlign: 'center' }" :exportable="false">
          <template #body="slotProps">
            <button class="btn-secondary btn-sm" @click="openFileModal(slotProps.data)" :disabled="!slotProps.data.evidence_files || slotProps.data.evidence_files === 0">
              보기
            </button>
          </template>
        </Column>
        <Column field="last_registered_at" header="최종 등록일시" :headerStyle="{ width: columnWidths.last_registered_at, textAlign: 'center' }" :bodyStyle="{ textAlign: 'center' }" :sortable="true">
          <template #body="slotProps">
      ...
      <div class="table-footer-wrapper"
        ...
        <table style="width:100%; table-layout:fixed;">
          <tr>
            <td style="width:48%; text-align:center; font-weight:600;">합계</td>
            <td style="width:8%; text-align:center; font-weight:600;">{{ totalClients }}</td>
            <td style="width:8%; text-align:center; font-weight:600;">{{ totalSubmittedClients }}</td>
            <td style="width:10%; text-align:center; font-weight:600;">{{ totalPrescriptionCount }}</td>
            <td style="width:12%; text-align:right; font-weight:600;">{{ formatNumber(totalPrescriptionAmount) }}</td>
            <td style="width:14%; text-align:center; font-weight:600;"></td>
          </tr>
        </table>
      </div>
    </div>
  ...
  <div v-if="showFileModal" class="modal-overlay" @click="closeFileModal">
    <div class="modal-content" @click.stop style="width: 80%; max-width: 1000px; max-height: 80vh;">
      ...
      <div class="modal-body" style="max-height: 60vh; overflow-y: auto;">
        <div v-if="fileLoading" class="loading-message">
          파일 목록을 불러오는 중...
        </div>
          ...
          <table class="file-list-table" style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr style="background: #f8f9fa; border-bottom: 2px solid #dee2e6;">
                <th style="width: 50%; padding: 12px; text-align: left; border-right: 1px solid #dee2e6;">거래처명</th>
                <th style="width: 50%; padding: 12px; text-align: left;">파일명</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="file in companyFiles" :key="file.id" style="border-bottom: 1px solid #dee2e6;">
                <td style="width: 50%; padding: 12px; border-right: 1px solid #dee2e6; vertical-align: top;">
      ...
      <div class="modal-footer">
        <button class="btn-secondary" @click="downloadAllFiles" :disabled="companyFiles.length === 0">
          전체 다운로드
        </button>
        <button class="btn-primary" @click="closeFileModal">
          닫기
        </button>
      </div>
    </div>
  ...
  try {
    ...
    for (const company of companiesData) {
      ...
      try {
        ...
        console.log(`Company ${company.company_name} - Total clients (count):`, totalClientCount, 'Error:', clientCountError)

        // 해당 업체의 실적 데이터 필터링
        const companyPerformances = performanceData?.filter(p => p.company_id === company.id) || []
        
        console.log(`Company ${company.company_name} - Performance records:`, companyPerformances.length)
        console.log(`Company ${company.company_name} - Sample performance:`, companyPerformances[0])
        
        // 제출 거래처 수 (중복 제거)
        const submittedClientIds = new Set(companyPerformances.map(p => p.client_id).filter(id => id))
        const submittedClients = submittedClientIds.size
        
        // 처방건수, 처방액 계산
        const prescriptionCount = companyPerformances.length
        const prescriptionAmount = companyPerformances.reduce((sum, p) => sum + ((p.prescription_qty || 0) * (p.products?.price || 0)), 0)
        ...
        try {
          ...
          if (tableTestError) {
            ...
            console.log(`📋 Supabase 대시보드에서 performance_evidence_files 테이블에 실제 데이터가 있는지 확인해주세요.`)
          ...
          console.log(`Company ${company.company_name} - 증빙 파일 조회 결과:`, companyFiles?.length || 0)
        ...
        if (companyPerformances.length > 0) {
          ...
          console.log(`Company ${company.company_name} - Latest performance:`, sortedPerformances[0])
          ...
          if (latestDate) {
            try {
              ...
              console.error(`Date parsing error for ${company.company_name}:`, dateError, latestDate)
            }
          }
        }
...
// 합계 계산
const totalClients = computed(() => companyList.value.reduce((sum, c) => sum + (c.total_clients || 0), 0))
const totalSubmittedClients = computed(() => companyList.value.reduce((sum, c) => sum + (c.submitted_clients || 0), 0))
const totalPrescriptionCount = computed(() => companyList.value.reduce((sum, c) => sum + (c.prescription_count || 0), 0))
const totalPrescriptionAmount = computed(() => companyList.value.reduce((sum, c) => sum + (c.prescription_amount || 0), 0))
...
const fetchCompanyFiles = async (company) => {
  ...
  try {
    ...
    if (realError) {
      ...
      if (realError.message.includes('RLS') || realError.message.includes('policy') || realError.message.includes('permission')) {
    ...
    companyFiles.value = realFiles.map(file => ({
      ...
      clients: clientsMap[file.client_id] || { name: file.client_id ? `거래처 ${file.client_id}` : '미지정' },
