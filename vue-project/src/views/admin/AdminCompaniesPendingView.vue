  ...
  <div class="admin-companies-view page-container">
    ...
    <div class="data-card">
      ...
      >
        ...
        <Column field="approval_status" header="승인 처리" :headerStyle="{ width: columnWidths.approval_status, textAlign: 'center' }" :exportable="false">
          <template #body="slotProps">
...
const fetchCompanies = async () => {
  ...
  try {
    ...
    // 미승인 업체만 필터링하고 병의원 수 추가
    pendingCompanies.value = (data || [])
      .filter(company => company.user_type === 'user' && company.approval_status === 'pending')
      .map(company => ({
        ...company,
...
const confirmApprovalChange = async (company, newStatus) => {
  ...
  const confirmed = await showConfirm(
    `정말 ${company.company_name} 업체를 ${actionText} 처리하시겠습니까?`,
    `${actionText} 확인`
  )
  if (!confirmed) return
  try {
