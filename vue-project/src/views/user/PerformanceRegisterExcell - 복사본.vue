      ...
      <DataTable :value="previewRows" scrollable scrollHeight="calc(100vh - 350px)" class="custom-table performance-list-table" style="margin-top:2rem;">
        ...
        <Column field="거래처" header="거래처" :headerStyle="{ width: '12%' }">
          <template #body="slotProps">
            <Dropdown v-model="slotProps.data['거래처']" :options="myClientsList" optionLabel="name" optionValue="name" placeholder="선택" style="width:100%" />
          </template>
        </Column>
        ...
        <Column field="수량" header="수량" :headerStyle="{ width: '7%' }">
          <template #body="slotProps">
            <InputText v-model.number="slotProps.data['수량']" @input="onPreviewQtyChange(slotProps.data, slotProps.index)" style="width:100%" />
          </template>
        </Column>
...
function onPreviewQtyChange(row, idx) {
  if (row['약가'] && row['수량']) {
    row['처방액'] = Number(row['약가']) * Number(row['수량']);
  } else {
    row['처방액'] = '';
  }
}
