  ...
  <div class="performance-register-view">
    ...
    <div class="table-container" style="position:relative;">
      <div style="display:flex;gap:32px;margin-bottom:12px;align-items:center;">
        ...
        <!-- 제품 선택 버튼 -->
        <button class="btn-product-select" @click="openProductModalForAdd" :disabled="!isInputEnabled">제품 선택</button>
      </div>
      ...
      <div style="margin-top:2rem;">
        <table style="width:100%;border-top:1px solid #222;">
          <tr>
            <td style="width:80px;">합계액</td>
...
function focusField(rowIdx, col) {
  nextTick(() => {
    ...
    if (!row) return;
    let el = null;
    if (col === 'product_name') el = row.querySelector('td:nth-child(1) input');
    else if (col === 'prescription_qty') el = row.querySelector('td:nth-child(4) input');
    else if (col === 'prescription_type') el = row.querySelector('td:nth-child(6) select');
    else if (col === 'remarks') el = row.querySelector('td:nth-child(7) input');
    if (el) el.focus();
  });
}
...
function addOrFocusNextRow(rowIdx) {
  if (rowIdx === inputRows.value.length - 1) {
    inputRows.value.push({ 
      ...
      prescription_qty: '', 
      prescription_amount: '', 
      prescription_type: inputRows.value[rowIdx].prescription_type || '', 
      remarks: '' 
    });
    nextTick(() => focusField(rowIdx + 1, 'product_name'));
  } else {
