...
async function fetchAllDataForMonth() {
  ...
  let assignmentHistoryMap = new Map(); // `${hospitalId}_${companyId}` -> [{effective_from_month, effective_to_month}]
  ...
  if (hospitalIds.length > 0 && companyId.value) {
    ...
    const { data: hospitalPerf, error: hospitalPerfError } = await supabase
      ...
      // 이관 연속성: 특정 업체로 한정하지 않고 병원+제품 단위 대상 자격(자격 있는 행)을 모두 조회
        ...
        if (!promotionError && promotionProducts) {
          ...
          hospitalPerf.forEach(hp => {
            ...
            if (promotionProduct) {
              ...
              // 병원+제품 단위 키 (업체 무관). 적용 업체 판정은 isPromotionApplicableToCompany 에서 처리
              ...
              hospitalPerformanceMap.set(key, {
                ...
                first_performance_cso_id: hp.first_performance_cso_id
              });
            }
          });
        }
  ...
  allDataForMonth.value = data.map(row => {
    ...
    if (productId) {
      ...
      if (insuranceCode) {
        ...
        const isAssigned = isAssignedForMonth(assignmentHistoryMap.get(`${hospitalId}_${companyId.value}`), selectedMonth.value);
        const isPromotionApplicable = promotionInfo
          && isPromotionApplicableToCompany(promotionInfo.first_performance_cso_id, companyId.value, selectedMonth.value, isAssigned, isNewCsoCompany);
    ...
    const isSmallZero = isSmallClientZeroApplicable(selectedMonth.value, row.prescription_month, ccTotal, getClientFirstMonth(clientFirstMonthMap, row.client_id), companyGroup);
