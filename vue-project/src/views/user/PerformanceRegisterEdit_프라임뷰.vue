      ...
      >
        ...
        <Column field="prescriptionMonth" header="처방월" :headerStyle="{ width: columnWidths.prescriptionMonth }">
          <template #body="slotProps">
            <Dropdown v-model="slotProps.data.prescriptionMonth" :options="prescriptionMonths" placeholder="선택" style="width:100%" @change="setChange"/>
          </template>
        </Column>
          ...
          <template #body="slotProps">
            <div class="p-fluid" style="position: relative;">
              <InputText
                ...
                @focus="onProductFocus(slotProps.index)"
                @blur="onProductBlur"
                :id="`product-input-${slotProps.index}`"
                placeholder="제품명/보험코드 검색"
                autocomplete="off"
              />
              <ul v-if="productSuggestions.length > 0 && activeProductSearchRow === slotProps.index" class="product-search-dropdown">
                <li v-for="(p, i) in productSuggestions" :key="p.id" @mousedown.prevent="onProductSelect(slotProps.data, p, slotProps.index)" :class="{ 'selected': i === productSuggestionIndex }">
  ...
  if (records && records.length > 0) {
    rows.value = records.map(record => {
      ...
      return {
        ...
        productNameQuery: record.products?.product_name || '',
...
function onProductSelect(row, value, rowIdx) {
  ...
  row.productNameQuery = value.product_name;
  productSuggestions.value = [];
  productSuggestionIndex.value = -1;
  activeProductSearchRow.value = null;
  setChange();
...
function onProductFocus(rowIdx) {
  setCellFocus(rowIdx, 'product');
  activeProductSearchRow.value = rowIdx;
}

function onProductBlur() {
  setTimeout(() => {
    if (activeProductSearchRow.value !== null) {
      productSuggestions.value = [];
      activeProductSearchRow.value = null;
      productSuggestionIndex.value = -1;
    }
...
function onProductKeydown(e, rowIdx, rowData) {
  ...
  if (e.key === 'ArrowDown') {
    ...
    e.preventDefault();
    if (productSuggestionIndex.value !== -1 && productSuggestions.value.length > 0) {
