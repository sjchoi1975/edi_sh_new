-- absorption_analysis 테이블 스키마 완전 정리 (데이터 타입 수정)

-- 1. 필요한 ID 컬럼들 추가 (올바른 데이터 타입으로)
-- client_id는 bigint (clients.id가 bigint이므로)
ALTER TABLE public.absorption_analysis 
ADD COLUMN IF NOT EXISTS client_id bigint NULL;

-- product_id는 uuid (products.id가 uuid이므로)
ALTER TABLE public.absorption_analysis 
ADD COLUMN IF NOT EXISTS product_id uuid NULL;

-- 2. 외래키 제약조건 추가 (존재 여부 확인 후)
-- client_id 외래키 (bigint → bigint)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- company_id 외래키 (uuid → uuid, 이미 존재할 수 있음)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- product_id 업데이트 (product_name 기반 매칭)
UPDATE public.absorption_analysis 
...
-- company_id 업데이트 (company_name 기반 매칭)
UPDATE public.absorption_analysis 
...
AND column_name IN ('company_id', 'client_id', 'product_id')
