-- 흡수율 계산 함수에 반올림 처리 추가
-- wholesale_revenue, direct_revenue, total_revenue: 정수로 반올림
-- absorption_rate: 소수점 3자리로 반올림

-- 1. 기존 함수 삭제
DROP FUNCTION IF EXISTS public.calculate_absorption_rates(text);

-- 2. 반올림 처리가 포함된 새로운 함수 생성
        ...
        SET 
            ...
            absorption_rate = CASE 
                WHEN result_record.prescription_amount > 0 THEN 
                    ROUND((COALESCE(result_record.wholesale_revenue, 0) + COALESCE(result_record.direct_revenue, 0)) / result_record.prescription_amount, 3)
                ELSE 0 
            END
        WHERE id = result_record.absorption_analysis_id;
    END LOOP;
    
    RAISE NOTICE '흡수율 분석 완료 (반올림 처리 포함): %월', p_settlement_month;
END;
$$;
