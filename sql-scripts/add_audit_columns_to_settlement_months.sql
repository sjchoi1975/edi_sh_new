-- settlement_months 테이블에 감사 정보 컬럼 추가

-- 1. created_by 컬럼 추가 (이미 있다면 무시)
ALTER TABLE settlement_months ADD COLUMN IF NOT EXISTS created_by UUID;

-- 2. updated_by 컬럼 추가 (이미 있다면 무시)
ALTER TABLE settlement_months ADD COLUMN IF NOT EXISTS updated_by UUID;

-- 3. updated_at 컬럼 추가 (이미 있다면 무시)
ALTER TABLE settlement_months ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE;
...
CREATE TRIGGER update_settlement_months_updated_at
    BEFORE UPDATE ON settlement_months
    FOR EACH ROW
